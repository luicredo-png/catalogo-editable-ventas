...
export async function GET(request:Request){
 ...
 // the first paint of the catalog considerably.
 if(slug==='gmpaonyx'){
  const imported=await env.DB.prepare("SELECT COUNT(*) AS count FROM products WHERE store_id=? AND name GLOB 'GMP-[0-9][0-9]: *'").bind(store.id).first<{count:number}>();
  if(Number(imported?.count||0)<26)await ensureGmpaonyxProducts(Number(store.id));
 }
