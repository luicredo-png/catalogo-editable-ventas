...
export async function GET(request:Request){
 ...
 const rows=await env.DB.prepare('SELECT id,name,category,description,price,old_price AS oldPrice,image,options_json AS optionsJson,whatsapp_message AS whatsappMessage,active,audience FROM products WHERE store_id=? AND active=1 ORDER BY sort_order,id').bind(store.id).all();
