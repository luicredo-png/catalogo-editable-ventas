...
export async function PUT(request:Request,{params}:{params:Promise<{id:string}>}){
 ...
 await env.DB.prepare('UPDATE products SET name=?,category=?,description=?,price=?,old_price=?,image=?,options_json=?,whatsapp_message=?,active=?,audience=? WHERE id=?').bind(String(p.name),String(p.category),String(p.description),Number(p.price),Number(p.oldPrice),String(p.image),JSON.stringify(Array.isArray(p.options)?p.options:[]),String(p.whatsappMessage||'').slice(0,1200),p.active?1:0,audience,Number(id)).run();
