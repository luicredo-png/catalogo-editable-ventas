...
export async function PUT(request:Request){
 ...
 const surfaceStyle=['solid','gradient','glass','media'].includes(String(s.surfaceStyle))?String(s.surfaceStyle):'solid';
 ...
 if(result.meta.changes)await env.DB.prepare('UPDATE stores SET whatsapp_message=? WHERE id=? AND owner_id=?').bind(whatsappMessage,Number(s.id),PUBLIC_OWNER).run();
