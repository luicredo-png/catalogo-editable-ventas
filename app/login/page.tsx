...
export default function Login(){
 const [error,setError]=useState(''),[busy,setBusy]=useState(false),[tenantSlug,setTenantSlug]=useState(''),[demoMode,setDemoMode]=useState(false),[landingMode,setLandingMode]=useState(false);
 useEffect(()=>{const host=location.hostname.toLowerCase(),siteHost=host.endsWith('.sitioweb.shop')&&!['www.sitioweb.shop','creador.sitioweb.shop'].includes(host),websiteDemo=host==='demo.sitioweb.shop';setDemoMode(websiteDemo||['xn--micatlogo-41a.shop','www.xn--micatlogo-41a.shop','micatalago.shop','www.micatalago.shop','micatalogo.shop','www.micatalogo.shop'].includes(host));setLandingMode(siteHost||(new URLSearchParams(location.search).get('next')||'').startsWith('/estudio-contable'));const match=host.match(/^([a-z0-9][a-z0-9-]{1,43}[a-z0-9])\.(?:xn--micatlogo-41a|sitioweb)\.shop$/);if(match&&!['www','creador'].includes(match[1]))setTenantSlug(match[1])},[]);
 async function submit(event:FormEvent<HTMLFormElement>){event.preventDefault();setBusy(true);setError('');const form=new FormData(event.currentTarget);try{const response=await fetch('/api/auth/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({identifier:demoMode?'demo':tenantSlug||form.get('identifier'),password:form.get('password')})});if(!response.ok){setError(response.status===503?'Falta configurar el acceso seguro. Contacta al propietario.':response.status===429?'Demasiados intentos. Espera 15 minutos.':'Contraseña incorrecta.');return}const requested=new URLSearchParams(location.search).get('next')||'/admin';const next=requested.startsWith('/')&&!requested.startsWith('//')?requested:'/admin';location.replace(next)}catch{setError('No se pudo conectar. Inténtalo nuevamente.')}finally{setBusy(false)}}
 return <main className="login-page">
  <form className="shine-login-card" onSubmit={submit}>
   <div className="login-heading">
    <small>ACCESO SEGURO</small>
    <h1>{landingMode?'Administrar landing':'Administrar mi catálogo'}</h1>
    <p>{landingMode?(tenantSlug?'Ingresa con la contraseña creada para administrar este sitio web.':'Ingresa con la contraseña del demo para editar textos, fotos e iconos.'):demoMode?'Escribe la contraseña del demo para probar todo el panel.':tenantSlug?'Ingresa con la contraseña creada para este catálogo.':'Ingresa con el usuario y la contraseña de tu catálogo.'}</p>
   </div>
   {tenantSlug||demoMode?<input name="identifier" type="hidden" value={demoMode?'demo':tenantSlug}/>:<label>Usuario<input name="identifier" type="text" autoComplete="username" placeholder="Tu usuario" required /></label>}
