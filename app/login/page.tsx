...
export default function Login(){
 ...
 useEffect(()=>{const host=location.hostname.toLowerCase();setDemoMode(['xn--micatlogo-41a.shop','www.xn--micatlogo-41a.shop','micatalago.shop','www.micatalago.shop','micatalogo.shop','www.micatalogo.shop'].includes(host));setLandingMode((new URLSearchParams(location.search).get('next')||'').startsWith('/estudio-contable'));const match=host.match(/^([a-z0-9][a-z0-9-]{1,43}[a-z0-9])\.xn--micatlogo-41a\.shop$/);if(match&&!['www','creador'].includes(match[1]))setTenantSlug(match[1])},[]);
 ...
 return <main className="login-page">
  <form className="shine-login-card" onSubmit={submit}>
   <div className="login-heading">
    ...
    <p>{landingMode?'Ingresa con la contraseña del demo para editar textos, fotos e iconos.':demoMode?'Escribe la contraseña del demo para probar todo el panel.':tenantSlug?'Ingresa con la contraseña creada para este catálogo.':'Ingresa con el usuario y la contraseña de tu catálogo.'}</p>
   </div>
