...
export default function Login(){
 const [error,setError]=useState(''),[busy,setBusy]=useState(false),[tenantSlug,setTenantSlug]=useState(''),[demoMode,setDemoMode]=useState(false);
 useEffect(()=>{const host=location.hostname.toLowerCase();setDemoMode(['xn--micatlogo-41a.shop','www.xn--micatlogo-41a.shop','micatalago.shop','www.micatalago.shop','micatalogo.shop','www.micatalogo.shop'].includes(host));const match=host.match(/^([a-z0-9][a-z0-9-]{1,43}[a-z0-9])\.xn--micatlogo-41a\.shop$/);if(match&&!['www','creador'].includes(match[1]))setTenantSlug(match[1])},[]);
 ...
 return <main className="login-page">
  <form className="shine-login-card" onSubmit={submit}>
   <div className="login-heading">
    ...
    <p>{demoMode?'Escribe la contraseña del demo para probar todo el panel.':tenantSlug?'Ingresa con la contraseña creada para este catálogo.':'Ingresa con el usuario y la contraseña de tu catálogo.'}</p>
   </div>
