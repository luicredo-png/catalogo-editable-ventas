...
import SiteCreator from './site-creator';
...
export default function CreatorPanel({sitesOnly=false}:{sitesOnly?:boolean}) {
  ...
  useEffect(() => { if(!sitesOnly)load().catch(() => setMessage("No se pudo consultar los clientes.")); }, [sitesOnly]);
  ...
  async function changePassword(event: FormEvent<HTMLFormElement>, client: Client) {
    ...
    try {
      ...
      form.reset(); setPasswordClientId(null); setGeneratedPassword(""); setMessage(`Contraseña actualizada para ${client.name}. Sus sesiones anteriores fueron cerradas.`);
    } catch (error) { setMessage(error instanceof Error ? error.message : "No se pudo cambiar la contraseña."); }
    finally { setBusy(false); }
  }
  function generatePassword(event: React.MouseEvent<HTMLButtonElement>) {
    const form = event.currentTarget.closest("form");
    const passwordInput = form?.querySelector<HTMLInputElement>('input[name="password"]');
    const confirmationInput = form?.querySelector<HTMLInputElement>('input[name="confirmation"]');
