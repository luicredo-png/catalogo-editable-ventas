...
export async function authenticate(request: Request, config: AuthConfig): Promise<Administrator | null> {
 ...
 if (row.owner_email) {
  ...
  return {email:String(row.owner_email),subject:'owner',owner:true,demo:false,tenant:'',storeId:null};
 }
 if (!row.active || !row.slug || tenantFromHost(host) !== row.slug) return null;
 return {email:String(row.email),subject:String(row.user_id),owner:false,demo:false,tenant:String(row.slug),storeId:Number(row.store_id)};
}
