(() => {
  ...
  const applyMobileType = () => {
    ...
    .models-button>span:first-child{flex:1;min-width:0;text-align:left}.live-modal-media{position:relative;overflow:hidden}.live-modal-media>img{cursor:zoom-in}.live-video-hint{z-index:4;border:0;white-space:nowrap;cursor:pointer}
    ...
    .revolt-panel-head:has(.delivered-title){position:relative;justify-content:center;min-height:72px}.delivered-title{display:block;width:min(520px,76vw);max-height:120px;object-fit:contain;margin:0 auto;filter:drop-shadow(0 8px 22px #000)}.delivered-title~.revolt-panel-close{position:absolute;right:0;top:50%;transform:translateY(-50%)}.delivered-carousel{position:relative}.delivered-stage{overflow-x:auto;overflow-y:hidden;scroll-snap-type:x proximity;overscroll-behavior-x:contain;touch-action:pan-x;border-radius:20px;background:radial-gradient(circle at 50% 0,#222 0,#090909 62%,#030303 100%);padding:18px 0;box-shadow:inset 0 1px #ffffff14,0 20px 55px #0008;scrollbar-width:none}.delivered-stage::-webkit-scrollbar{display:none}.delivered-track{display:flex;align-items:center;gap:8px;width:max-content}.delivered-track button{display:block;flex:0 0 auto;scroll-snap-align:center;border:0;padding:0;margin:0;background:transparent;cursor:zoom-in}.delivered-track img{display:block;width:260px;height:300px;object-fit:cover;border-radius:16px;border:0;background:#080808;box-shadow:0 16px 36px #000b;transition:transform .35s ease,filter .35s ease}.delivered-track button:hover img{transform:translateY(-4px) scale(1.015);filter:brightness(1.04)}.delivered-scroll-button{position:absolute;z-index:3;top:50%;transform:translateY(-50%);display:grid;place-items:center;width:42px;height:64px;border:1px solid #ffffff55;border-radius:999px;background:#090909dc;color:#fff;font-size:28px;box-shadow:0 8px 26px #000b}.delivered-scroll-prev{left:8px}.delivered-scroll-next{right:8px}.delivered-note{margin:13px 8px 0;text-align:center;color:#fff;font-size:13px;font-weight:750;letter-spacing:.025em}
    .media-lightbox{position:fixed;inset:0;z-index:12000;display:grid;place-items:center;padding:18px;background:#000e;backdrop-filter:blur(10px);touch-action:pan-y}.media-lightbox-backdrop{position:absolute;inset:0;border:0;background:transparent}.media-lightbox-image{position:relative;z-index:1;display:block;max-width:min(1180px,96vw);max-height:92dvh;object-fit:contain;border-radius:16px;box-shadow:0 26px 90px #000}.media-lightbox-close,.media-lightbox-nav{position:fixed;z-index:3;display:grid;place-items:center;border:1px solid #ffffff6b;background:#111d;color:#fff;box-shadow:0 8px 28px #000b}.media-lightbox-close{top:max(14px,env(safe-area-inset-top));right:14px;width:46px;height:46px;border-radius:50%;font-size:30px}.media-lightbox-nav{top:50%;width:46px;height:64px;border-radius:999px;font-size:30px;transform:translateY(-50%)}.media-lightbox-prev{left:14px}.media-lightbox-next{right:14px}.media-lightbox-count{position:fixed;z-index:3;left:50%;bottom:max(14px,env(safe-area-inset-bottom));transform:translateX(-50%);padding:7px 12px;border-radius:999px;background:#111d;color:#fff;font-size:12px;font-weight:900}
    ...
    @media (max-width:650px){
      ...
      .brand-scroll-arrow{display:flex!important}.revoluciona-hero{width:94vw;max-height:150px;margin-top:0}.revolt-dock{left:10px;right:10px;bottom:10px;justify-content:space-between}.revolt-dock button{padding:10px 12px;font-size:12px}.delivered-title{width:76vw;max-height:82px}.delivered-stage{padding:10px 0}.delivered-track{gap:6px}.delivered-track button{margin:0}.delivered-track img{width:190px;height:225px;border-radius:13px}.delivered-scroll-button{width:34px;height:52px;font-size:24px}.delivered-scroll-prev{left:4px}.delivered-scroll-next{right:4px}.delivered-note{font-size:11px;line-height:1.4}.price-change-toast{top:50%;width:86vw;padding:10px;border-radius:15px}.price-change-toast img{max-height:30vh;border-radius:8px}.live-video-hint{right:10px!important;bottom:10px!important;max-width:calc(100% - 20px)!important;padding:8px 11px!important;font-size:11px!important;line-height:1!important}.media-lightbox{padding:0}.media-lightbox-image{max-width:100vw;max-height:100dvh;border-radius:0}.media-lightbox-nav{width:38px;height:54px;font-size:25px}.media-lightbox-prev{left:7px}.media-lightbox-next{right:7px}
  ...
  const addToCart = (product, index, source) => {
    ...
    document.querySelector('.price-change-toast')?.remove();
    const image = after === 1 ? promoArt.firstPair : after === 2 ? promoArt.secondPair : promoArt.seventyPair;
    const notice = after === 1 ? 'El siguiente par baja a S/80' : after === 2 ? 'Desde el tercer par cada pedido baja a S/70' : 'Producto agregado a solo S/70';
    document.body.insertAdjacentHTML('beforeend', `<div class="price-change-toast" role="status" aria-label="${notice}. Toca para cerrar"><img src="${image}" alt="${notice}"><span>${notice}</span></div>`);
    const toast = document.querySelector('.price-change-toast');
    const dismissToast = () => toast?.remove();
    setTimeout(() => document.addEventListener('pointerdown', dismissToast, { once:true, capture:true }), 0);
    setTimeout(dismissToast, 5300);
  };
  const renderDock = () => {
  ...
  const renderDeliveredPanel = () => {
    ...
    const show = images.length ? [...images, ...images].map((src, index) => `<button type="button" data-view-delivered="${index % images.length}" aria-label="Ampliar pedido ${index % images.length + 1}"><img src="${esc(src)}" alt="Pedido enviado en la semana"></button>`).join('') : '<p>Aquí aparecerán los pedidos enviados durante la semana.</p>';
    document.body.insertAdjacentHTML('beforeend', `<div class="revolt-panel" id="revolt-panel"><section class="revolt-panel-card"><header class="revolt-panel-head"><img class="delivered-title" src="${promoArt.deliveredTitle}" alt="Pedidos enviados esta semana"><button class="revolt-panel-close" type="button" data-panel-close>×</button></header><div class="delivered-carousel"><button class="delivered-scroll-button delivered-scroll-prev" type="button" data-delivered-scroll="-1" aria-label="Pedidos anteriores">‹</button><div class="delivered-stage"><div class="delivered-track">${show}</div></div><button class="delivered-scroll-button delivered-scroll-next" type="button" data-delivered-scroll="1" aria-label="Más pedidos">›</button></div><p class="delivered-note">Te enviaremos una foto de tu pedido alistado antes de enviártelo.</p></section></div>`);
    document.getElementById('revolt-panel').dataset.deliveredImages = JSON.stringify(images);
    const stage = document.querySelector('#revolt-panel .delivered-stage'), track = stage?.querySelector('.delivered-track');
    if (stage && track && images.length > 1) {
      const pause = () => { stage.dataset.pauseUntil = String(Date.now() + 4200); };
      stage.addEventListener('pointerdown', pause, { passive:true });
      stage.addEventListener('wheel', pause, { passive:true });
      let previous = performance.now();
      const advance = now => {
        if (!stage.isConnected) return;
        if (Date.now() >= Number(stage.dataset.pauseUntil || 0)) {
          stage.scrollLeft += Math.min(1.1, (now - previous) * .035);
          const half = track.scrollWidth / 2;
          if (half && stage.scrollLeft >= half) stage.scrollLeft -= half;
        }
        previous = now;
  ...
  const closeImageViewer = () => { document.getElementById('live-image-viewer')?.remove(); state.viewerImages = []; };
  const renderImageViewer = () => {
    document.getElementById('live-image-viewer')?.remove();
    if (!state.viewerImages.length) return;
    const total = state.viewerImages.length, index = ((state.viewerIndex % total) + total) % total;
    state.viewerIndex = index;
    document.body.insertAdjacentHTML('beforeend', `<div class="media-lightbox" id="live-image-viewer" role="dialog" aria-modal="true" aria-label="Imagen ampliada"><button class="media-lightbox-backdrop" type="button" data-viewer-close aria-label="Cerrar"></button><img class="media-lightbox-image" src="${esc(state.viewerImages[index])}" alt="Imagen ${index + 1} de ${total}"><button class="media-lightbox-close" type="button" data-viewer-close aria-label="Cerrar">×</button>${total > 1 ? `<button class="media-lightbox-nav media-lightbox-prev" type="button" data-viewer-step="-1" aria-label="Anterior">‹</button><button class="media-lightbox-nav media-lightbox-next" type="button" data-viewer-step="1" aria-label="Siguiente">›</button><span class="media-lightbox-count">${index + 1} / ${total}</span>` : ''}</div>`);
  };
  const openImageViewer = (images, index = 0) => { state.viewerImages = images.filter(Boolean); state.viewerIndex = index; renderImageViewer(); };
  ...
  const bind = () => {
    document.addEventListener('click', event => {
      const controlled = event.target.closest('[data-brand], .gender-tabs [data-gender], [data-thumb], [data-open], [data-close], [data-modal-thumb], [data-toggle-media], [data-view-product], [data-view-delivered], [data-delivered-scroll], [data-viewer-close], [data-viewer-step], [data-add-cart], [data-cart-open], [data-delivered-open], [data-panel-close], [data-cart-remove], [data-brand-scroll]');
      ...
      if (event.target.closest('[data-toggle-media]') && videoFor(state.modal, state.modalIndex)) { state.showVideo = !state.showVideo; renderModal(); return; }
      if (event.target.closest('[data-view-product]') && state.modal) { openImageViewer(anglesFor(state.modal, state.modalIndex), 0); return; }
      const delivered = event.target.closest('[data-view-delivered]');
      if (delivered) { const panel = delivered.closest('#revolt-panel'), images = JSON.parse(panel?.dataset.deliveredImages || '[]'); openImageViewer(images, Number(delivered.dataset.viewDelivered)); return; }
      const deliveredScroll = event.target.closest('[data-delivered-scroll]');
      if (deliveredScroll) { const stage = deliveredScroll.closest('.delivered-carousel')?.querySelector('.delivered-stage'); if (stage) { stage.dataset.pauseUntil = String(Date.now() + 4200); stage.scrollBy({ left:Number(deliveredScroll.dataset.deliveredScroll) * Math.min(560, innerWidth * .72), behavior:'smooth' }); } return; }
      if (event.target.closest('[data-viewer-close]')) { closeImageViewer(); return; }
      const viewerStep = event.target.closest('[data-viewer-step]');
      if (viewerStep) { state.viewerIndex += Number(viewerStep.dataset.viewerStep); renderImageViewer(); return; }
    ...
    document.addEventListener('touchstart', event => { if (event.target.closest('#live-image-viewer')) viewerTouchStart = event.changedTouches[0].clientX; }, { passive:true });
    document.addEventListener('touchend', event => { if (!viewerTouchStart || !event.target.closest('#live-image-viewer')) return; const delta = event.changedTouches[0].clientX - viewerTouchStart; viewerTouchStart = 0; if (Math.abs(delta) > 45 && state.viewerImages.length > 1) { state.viewerIndex += delta < 0 ? 1 : -1; renderImageViewer(); } }, { passive:true });
    document.addEventListener('keydown', event => { if (!document.getElementById('live-image-viewer')) return; if (event.key === 'Escape') closeImageViewer(); if (event.key === 'ArrowLeft' && state.viewerImages.length > 1) { state.viewerIndex -= 1; renderImageViewer(); } if (event.key === 'ArrowRight' && state.viewerImages.length > 1) { state.viewerIndex += 1; renderImageViewer(); } });
