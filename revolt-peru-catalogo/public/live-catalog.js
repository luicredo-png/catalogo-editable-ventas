(() => {
  ...
  const applyMobileType = () => {
    ...
    .revolt-panel-head:has(.delivered-title){position:relative;justify-content:center;min-height:72px}.delivered-title{display:block;width:min(520px,76vw);max-height:120px;object-fit:contain;margin:0 auto;filter:drop-shadow(0 8px 22px #000)}.delivered-title~.revolt-panel-close{position:absolute;right:0;top:50%;transform:translateY(-50%)}.delivered-carousel{position:relative}.delivered-stage{overflow-x:auto;overflow-y:hidden;scroll-behavior:smooth;scroll-snap-type:x proximity;overscroll-behavior-x:contain;touch-action:pan-x;border-radius:20px;background:radial-gradient(circle at 50% 0,#222 0,#090909 62%,#030303 100%);padding:18px 0;box-shadow:inset 0 1px #ffffff14,0 20px 55px #0008;scrollbar-width:none}.delivered-stage::-webkit-scrollbar{display:none}.delivered-track{display:flex;align-items:center;gap:0;width:max-content}.delivered-track button{display:block;flex:0 0 auto;scroll-snap-align:center;border:0;padding:0;margin:0 -8px;background:transparent;cursor:zoom-in}.delivered-track img{display:block;width:260px;height:300px;object-fit:cover;border-radius:16px;border:0;background:#080808;box-shadow:0 16px 36px #000b;transition:transform .35s ease,filter .35s ease}.delivered-track button:hover img{transform:translateY(-4px) scale(1.015);filter:brightness(1.04)}.delivered-scroll-button{position:absolute;z-index:3;top:50%;transform:translateY(-50%);display:grid;place-items:center;width:42px;height:64px;border:1px solid #ffffff55;border-radius:999px;background:#090909dc;color:#fff;font-size:28px;box-shadow:0 8px 26px #000b}.delivered-scroll-prev{left:8px}.delivered-scroll-next{right:8px}.delivered-note{margin:13px 8px 0;text-align:center;color:#fff;font-size:13px;font-weight:750;letter-spacing:.025em}
  ...
  const renderDeliveredPanel = () => {
    ...
    const show = images.length ? images.map((src, index) => `<button type="button" data-view-delivered="${index}" aria-label="Ampliar pedido ${index + 1}"><img src="${esc(src)}" alt="Pedido enviado en la semana"></button>`).join('') : '<p>Aquí aparecerán los pedidos enviados durante la semana.</p>';
  ...
  const bind = () => {
    document.addEventListener('click', event => {
      ...
      if (deliveredScroll) { deliveredScroll.closest('.delivered-carousel')?.querySelector('.delivered-stage')?.scrollBy({ left:Number(deliveredScroll.dataset.deliveredScroll) * Math.min(560, innerWidth * .72), behavior:'smooth' }); return; }
