(() => {
  ...
  const applyMobileType = () => {
    ...
    @media (max-width:650px){
      ...
      .brand-scroll-arrow{display:flex!important}.revoluciona-hero{width:94vw;max-height:150px;margin-top:0}.revolt-dock{left:10px;right:10px;bottom:10px;justify-content:space-between}.revolt-dock button{padding:10px 12px;font-size:12px}.delivered-title{width:76vw;max-height:82px}.delivered-stage{padding:12px 0}.delivered-track{gap:12px}.delivered-track img{width:220px;height:250px}.price-change-toast{top:50%;width:94vw;padding:8px;border-radius:16px}.price-change-toast img{max-height:34vh;border-radius:11px}
  ...
  const renderCartPanel = () => {
    ...
    document.body.insertAdjacentHTML('beforeend', `<div class="revolt-panel" id="revolt-panel"><section class="revolt-panel-card"><header class="revolt-panel-head"><h2>Tu carrito</h2><button class="revolt-panel-close" type="button" data-panel-close>×</button></header>${lines}${state.cart.length ? `<div class="modal-price cart-offer"><span class="cart-offer-copy"><b>Oferta hasta agotar stock: El SEGUNDO par a S/80, El TERCER par a S/70</b><em>Puede combinar TALLAS, MODELOS Y COLORES</em></span><strong data-cart-total>Total S/${total}</strong></div><a class="cart-send whatsapp-button" href="https://wa.me/${number}?text=${encodeURIComponent(message)}" target="_blank" rel="noreferrer"><span class="whatsapp-icon-image" aria-hidden="true"></span><span>Pedir carrito por Whatsapp</span></a>` : ''}</section></div>`);
  };
  const renderDeliveredPanel = () => {
    ...
    document.body.insertAdjacentHTML('beforeend', `<div class="revolt-panel" id="revolt-panel"><section class="revolt-panel-card"><header class="revolt-panel-head"><img class="delivered-title" src="${promoArt.deliveredTitle}" alt="Pedidos enviados esta semana"><button class="revolt-panel-close" type="button" data-panel-close>×</button></header><div class="delivered-stage"><div class="delivered-track" style="--delivered-speed:${speed}s">${show}</div></div></section></div>`);
  };
