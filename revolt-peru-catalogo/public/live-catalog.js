(() => {
  ...
  const applyMobileType = () => {
    ...
    .revolt-panel-head:has(.delivered-title){position:relative;justify-content:center;min-height:72px}.delivered-title{display:block;width:min(520px,76vw);max-height:120px;object-fit:contain;margin:0 auto;filter:drop-shadow(0 8px 22px #000)}.delivered-title~.revolt-panel-close{position:absolute;right:0;top:50%;transform:translateY(-50%)}.delivered-stage{overflow:hidden;border-radius:20px;background:radial-gradient(circle at 50% 0,#222 0,#090909 62%,#030303 100%);padding:18px 0;box-shadow:inset 0 1px #ffffff14,0 20px 55px #0008}.delivered-track{display:flex;align-items:center;gap:2px;width:max-content;animation:deliveredMarquee var(--delivered-speed,34s) linear infinite;will-change:transform}.delivered-track:hover{animation-play-state:paused}.delivered-track button{display:block;flex:0 0 auto;border:0;padding:0;background:transparent;cursor:zoom-in}.delivered-track img{display:block;width:280px;height:310px;object-fit:contain;border-radius:18px;border:0;background:transparent;box-shadow:0 16px 36px #000b;transition:transform .35s ease,filter .35s ease}.delivered-track button:hover img{transform:translateY(-4px) scale(1.015);filter:brightness(1.04)}.delivered-note{margin:13px 8px 0;text-align:center;color:#fff;font-size:13px;font-weight:750;letter-spacing:.025em}@keyframes deliveredMarquee{to{transform:translateX(-50%)}}
  ...
  const addToCart = (product, index, source) => {
    ...
    if (after >= 2) { document.querySelector('.price-change-toast')?.remove(); const image = after === 2 ? promoArt.secondPair : promoArt.seventyPair, notice = after === 2 ? 'Desde el tercer par cada pedido baja a S/70' : 'Producto agregado a solo S/70'; document.body.insertAdjacentHTML('beforeend', `<div class="price-change-toast" role="status"><img src="${image}" alt="${notice}"><span>${notice}</span></div>`); setTimeout(()=>document.querySelector('.price-change-toast')?.remove(),5300); }
  };
  const renderDock = () => {
  ...
  const bind = () => {
    document.addEventListener('click', event => {
      const controlled = event.target.closest('[data-brand], .gender-tabs [data-gender], [data-thumb], [data-open], [data-close], [data-modal-thumb], [data-toggle-media], [data-view-product], [data-view-delivered], [data-viewer-close], [data-viewer-step], [data-add-cart], [data-cart-open], [data-delivered-open], [data-panel-close], [data-cart-remove], [data-brand-scroll]');
