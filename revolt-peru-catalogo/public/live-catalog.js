(() => {
  ...
  const applyMobileType = () => {
    ...
    .delivered-stage{overflow:hidden;border-radius:14px;background:#111;padding:14px}.delivered-track{display:flex;gap:14px;width:max-content;animation:deliveredMarquee 26s linear infinite}.delivered-track img{width:260px;height:260px;object-fit:cover;border-radius:14px;border:1px solid #444}@keyframes deliveredMarquee{to{transform:translateX(-50%)}}
  ...
  const saveCart = () => { localStorage.setItem('revolt-cart', JSON.stringify(state.cart)); renderDock(); };
  const addToCart = (product, index) => {
    const key = `${product.id}:${index}`;
    const found = state.cart.find(item => item.key === key);
    if (found) found.qty += 1;
    else state.cart.push({ key, id: product.id, index, qty: 1, name: product.name, brand: product.brand, price: product.price, image: photos(product)[index] || '/assets/logo.png' });
    saveCart();
  };
  const renderDock = () => {
    let dock = document.getElementById('revolt-dock');
    if (!dock) {
      document.body.insertAdjacentHTML('beforeend', '<nav class="revolt-dock" id="revolt-dock" aria-label="Acciones rápidas"><button type="button" data-cart-open>🛒 Carrito <span class="cart-count">0</span></button><button type="button" data-delivered-open>✓ Pedidos entregados</button></nav>');
  ...
  const renderCartPanel = () => {
    closePanel();
    const number = String(state.data.settings.whatsapp || '51981395069').replace(/\D/g, '');
    const message = ['Hola, quiero pedir:', ...state.cart.map(item => `• ${item.qty}x ${item.name} - color ${item.index + 1} - S/${Math.round(item.price)}`)].join('\n');
    const lines = state.cart.length ? state.cart.map(item => `<div class="cart-line"><img src="${esc(item.image)}" alt=""><div><strong>${esc(item.name)}</strong><small>${esc(item.brand)} · Color ${item.index + 1} · Cant. ${item.qty}</small></div><button type="button" data-cart-remove="${esc(item.key)}">Quitar</button></div>`).join('') : '<p>Tu carrito está vacío.</p>';
    document.body.insertAdjacentHTML('beforeend', `<div class="revolt-panel" id="revolt-panel"><section class="revolt-panel-card"><header class="revolt-panel-head"><h2>Tu carrito</h2><button class="revolt-panel-close" type="button" data-panel-close>×</button></header>${lines}${state.cart.length ? `<a class="cart-send" href="https://wa.me/${number}?text=${encodeURIComponent(message)}" target="_blank">Pedir carrito por Whatsapp</a>` : ''}</section></div>`);
  };
  const renderDeliveredPanel = () => {
    closePanel();
    const images = (state.data.settings.deliveredImages || []).filter(Boolean).map(mediaUrl);
    const show = images.length ? [...images, ...images].map(src => `<img src="${esc(src)}" alt="Pedido entregado">`).join('') : '<p>Aquí aparecerán los pedidos preparados y entregados.</p>';
    document.body.insertAdjacentHTML('beforeend', `<div class="revolt-panel" id="revolt-panel"><section class="revolt-panel-card"><header class="revolt-panel-head"><h2>Pedidos entregados</h2><button class="revolt-panel-close" type="button" data-panel-close>×</button></header><div class="delivered-stage"><div class="delivered-track">${show}</div></div></section></div>`);
  };
  ...
  const bind = () => {
    document.addEventListener('click', event => {
      ...
      if (add) { const [id, index] = add.dataset.addCart.split(':').map(Number); const product = state.data.products.find(p => p.id === id); if (product) addToCart(product, index); return; }
